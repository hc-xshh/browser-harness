...
def test_recordings_status_reports_default_off(monkeypatch, tmp_path, capsys):
...
def test_recordings_enable_persists_preference(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv("BH_AGENT_WORKSPACE", str(tmp_path))
    monkeypatch.setenv("BH_CONFIG_DIR", str(tmp_path / "config"))
    monkeypatch.delenv("BH_RECORD", raising=False)

    run._run(["recordings", "enable"])
    assert capsys.readouterr().out.strip() == "auto-recording preference enabled"
    run._run(["recordings"])
    assert capsys.readouterr().out.splitlines()[0] == "auto-recording: on (config)"

    run._run(["recordings", "disable"])
    assert capsys.readouterr().out.strip() == "auto-recording preference disabled"
    run._run(["recordings"])
    assert capsys.readouterr().out.splitlines()[0] == "auto-recording: off (config)"


def test_recordings_latest_prints_canonical_path(monkeypatch, tmp_path, capsys):
    recording = tmp_path / "recordings" / "session-1"
    recording.mkdir(parents=True)
    (recording / "events.jsonl").write_text("{}\n")
    monkeypatch.setenv("BH_AGENT_WORKSPACE", str(tmp_path))

    run._run(["recordings", "--latest"])

    assert capsys.readouterr().out.strip() == str(recording)


def test_recordings_latest_fails_when_empty(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv("BH_AGENT_WORKSPACE", str(tmp_path))
